...
\t─║║║║║─║║║║║║║─║║║║╚╗║╠══╗║─║║─║╚═╝║
\t╔╝╚╝║╚═╝║║║║║║╔╣╠╣║─║║║╚═╝║─║║─║╔═╗║
\t╚═══╩═══╩╝╚╝╚╝╚══╩╝─╚═╩═══╝─╚╝─╚╝─╚╝\n\t{P2}        CRACK INSTAGRAM BY BIMZ""",title=f"{P2}{H2}{self.timeLine()}",width=80,padding=(0,4),style=f"{color_table}"))
...
{N}[{H}+{N}] {H}•• LIVE ••{N}
 ...
 {N}├╴>{N} following : {H}{ful}{N}
    ...
    def crakersAPI(self, user, pasw):
        prog.update(des,description=f"[{H2} BIMZ-XD {P2}] ( {H2}stabil{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
        prog.advance(des)
